...
def init_tools(mcp: FastMCP):
    ...
    @mcp.tool()
    async def get_aks_clusters(subscription: str) -> str:
        """Get Azure AKS clusters using the Azure CLI.

        Args:
            state: Filter parameter (can be resource group or any filter criteria)
        """
        logger.info("Fetching AKS clusters with subscription: %s", subscription)
        try:
            # Run the az aks list command
            cmd = ["az", "aks", "list"]
            # Check if state parameter is a subscription filter
            if subscription:
